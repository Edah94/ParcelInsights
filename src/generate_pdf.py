...
def create_pdf(gnr_input, kg_input, output_path):
    ...
    for layer in layers_list:
        ...
        if layer == "gdf_parcel":
            ...
            # Create the content flowables
            image = Image(output_path_parcel, width=440, height=440)
        ...
        if layer == "gdf_geology":
            ...
            intro = """Geology can provide invaluable information about specific piece of land, aiding in understanding its prast, present and potential future.
            ...
            if num_rows == 1:
                ...
                referred to as '<b>{}</b>'. This geological formation covers <b>{}</b>% of the total parcel area, providing unique soil and 
                groundwater characteristics that may impact the use and management of the land.""".format(num_rows, geology_code_value[0], geology_formation_value[0], geology_lithomain_value[0], geology_lithonebe_value[0], geology_kurztitel_value[0], geology_coverage_value[0])
            else:
                iterator = 0
            ...
            # Create the content flowables
            image = Image(output_path_geology, width=440, height=440)
            table_data = [list(data.columns)] + [list(row) for row in data.values]
        ...
        if layer == "gdf_riskzones_water":
            ...
            image = Image(output_path_riskzones, width=440, height=440)
            table_data = [list(data.columns)] + [list(row) for row in data.values]
            table = Table(table_data, style=table_style) #colWidths=colWidths
        ...
        if layer == "dem":
            ...
            # Create the content flowables
            if wms_hillshade_available == "yes":
                image = Image(output_path_hillshade, width=440, height=440)
            else:
                image = Image(output_path_dem_png, width=440, height=440)
            table_data = [list(data.columns)] + [list(row) for row in data.values]
            table = Table(table_data, style=table_style) #colWidths=colWidths
        ...
        if layer == "aspect":
            ...
            # Create the content flowables
            image = Image(output_path_aspect, width=440, height=440)
            table_data = [list(data.columns)] + [list(row) for row in data.values]
            table = Table(table_data, style=table_style) #colWidths=colWidths
        ...
        if layer == "slope":
            ...
            # Create the content flowables
            image = Image(output_path_slope, width=440, height=440)
            table_data = [list(data.columns)] + [list(row) for row in data.values]
            table = Table(table_data, style=table_style) #colWidths=colWidths
